...
final class CominaviCatalogTests: XCTestCase {
    func testCatalogDownloadRedirectPolicyAllowsOnlyExactCapabilityHostAndStripsBearer() throws {
        let originalURL = try XCTUnwrap(
            URL(string: "https://cominavi.net/api/v2/catalogs/108/versions/v1/artifact")
        )
        ...
        let token = "1800000000-" + String(repeating: "A", count: 43) + "="
        let capabilityURL = try XCTUnwrap(
            URL(string: "https://catalogs.cominavi.net/derived/catalogs/c108/v1.sqlite?verify=\(token)")
        )
        var proposed = URLRequest(url: capabilityURL)
        proposed.setValue("Bearer secret", forHTTPHeaderField: "Authorization")

        let allowed = CatalogDownloadRedirectPolicy.redirectedRequest(
            original: original,
            response: response,
            proposed: proposed
        )

        XCTAssertEqual(allowed?.url, capabilityURL)
        XCTAssertNil(allowed?.value(forHTTPHeaderField: "Authorization"))
        XCTAssertFalse(
            CatalogDownloadRedirectPolicy.isAllowedCapabilityURL(
                try XCTUnwrap(
                    URL(string: "https://evil.example/derived/catalogs/c108/v1.sqlite?verify=\(token)")
                )
            )
        )
        XCTAssertFalse(
            CatalogDownloadRedirectPolicy.isAllowedCapabilityURL(
                try XCTUnwrap(
                    URL(string: "https://catalogs.cominavi.net/raw/catalogs/c108/main.sqlite?verify=\(token)")
                )
            )
        )
        let r2URL = try XCTUnwrap(
            URL(
                        string: "https://bee683f3b5473a422feaa41e040ac176.r2.cloudflarestorage.com/cominavi-catalog-downloads/derived/catalogs/c108/v1.sqlite?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=fixture%2F20260812%2Fauto%2Fs3%2Faws4_request&X-Amz-Date=20260812T000000Z&X-Amz-Expires=300&X-Amz-SignedHeaders=host&X-Amz-Signature=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
            )
        )
        XCTAssertTrue(CatalogDownloadRedirectPolicy.isAllowedCapabilityURL(r2URL))
        XCTAssertFalse(
            CatalogDownloadRedirectPolicy.isAllowedCapabilityURL(
                try XCTUnwrap(
                    URL(
                        string: "https://bee683f3b5473a422feaa41e040ac176.r2.cloudflarestorage.com/cominavi-catalogs-private/raw/catalogs/c108/main.sqlite?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Expires=300&X-Amz-Signature=fixture"
                    )
                )
            )
        )
        XCTAssertFalse(
            CatalogDownloadRedirectPolicy.isAllowedCapabilityURL(
                try XCTUnwrap(
                    URL(
                        string: "https://bee683f3b5473a422feaa41e040ac176.r2.cloudflarestorage.com/cominavi-catalog-downloads/derived/catalogs/c108/v1.sqlite?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=fixture%2F20260812%2Fauto%2Fs3%2Faws4_request&X-Amz-Date=20260812T000000Z&X-Amz-Expires=300&X-Amz-SignedHeaders=host&X-Amz-Signature=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa&X-Amz-Signature=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
                    )
                )
            )
        )
    ...
    func testRangeDownloadAcceptsResponseWithoutAdvisoryAcceptRangesHeader() async throws {
        let root = try temporaryDirectory(named: "range-proxy-headers")
        defer { try? FileManager.default.removeItem(at: root) }
        let fixture = try makeCatalogDatabase(in: root, versionID: "c108-proxy-v1")
        let transport = CatalogTransportStub(
            artifact: try Data(contentsOf: fixture.databaseURL),
            catalog: fixture.catalog,
            omitsAcceptRanges: true
        )
        let installer = CominaviCatalogInstaller(
            authorizer: CatalogAuthorizerStub(),
            transport: transport,
...
private actor CatalogTransportStub: CominaviCatalogDownloadTransporting {
    ...
    private let omitsAcceptRanges: Bool
    ...
    init(
        ...
        omitsAcceptRanges: Bool = false
    ) {
        self.artifact = artifact
        self.catalog = catalog
        self.steps = steps
        self.omitsContentLength = omitsContentLength
        self.omitsAcceptRanges = omitsAcceptRanges
    }
